...
SPIClass DW1000RangingClass::_spi;
...
void DW1000RangingClass::initCommunication(uint8_t myRST, uint8_t mySS, uint8_t myIRQ, SPIClass spi,uint8_t connection_num) {
	// reset line to the chip
	_RST              = myRST;
	_SS               = mySS;
	...
	if(connection_num == 2){
		DW2_1000.begin(myIRQ, myRST , spi);
		DW2_1000.select(mySS);
	}
